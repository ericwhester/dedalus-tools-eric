...
def plot1D(x,arr,
    ...
    ax.set(xlim=xlim,ylim=ylim,aspect=aspect,xlabel=xlabel,ylabel=ylabel,title=title)
...
def polar_plot(θθ,rr,array,
               fig=None,ax=None,save=False,colorbar=True,
    ...
    if save != False: plt.savefig(save,dpi=400,bbox_inches='tight')
    if return_plot: return fig, ax, plot
    return fig, ax    
    
def polar_vel_plot(θθ,rr,ur, uθ,
                   sel=None,fig=None,ax=None,save=False,
